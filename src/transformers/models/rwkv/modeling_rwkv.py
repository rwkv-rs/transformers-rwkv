...
class RwkvTimeMix(nn.Module):
    ...
    def _inference_forward(
        ...
        receptance = linear(xr, self.receptance.weight.contiguous())
        key = linear(xk, self.key.weight.contiguous())
        value = linear(xv, self.value.weight.contiguous())
        ...
        output = linear(output, self.output.weight.contiguous()).view(batch_size, sequence_length, channels)
...
class RwkvBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states + self.ffn(
            self.ln2(hidden_states), past_key_values=past_key_values, attention_mask=attention_mask
        )
        if past_key_values is not None:
...
class RwkvForCausalLM(RwkvPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if not return_dict:
            return tuple(value for value in (loss, logits, outputs.past_key_values) if value is not None)
