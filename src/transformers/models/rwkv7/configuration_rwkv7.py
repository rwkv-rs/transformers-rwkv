...
class Rwkv7Config(PreTrainedConfig):
    ...
    embedding_layer_norm_fused (`bool`, *optional*, defaults to `False`):
        Whether the block-0 layer normalization has already been fused into the embedding table. This must only be
        enabled for checkpoints converted with embedding layer-normalization fusion. This is an inference conversion
        detail and is unrelated to RWKV-8 DeepEmbed.
    wkv_backend (`str`, *optional*, defaults to `"auto"`):
        WKV execution backend. `"auto"` dispatches to the bundled vllm-rwkv inference kernel in evaluation mode and
        the official RWKV-v7 wind-backstepping kernel in training mode. Additional backends can be registered before
        model construction.
    wkv_state_dtype (`str`, *optional*, defaults to `"float32"`):
        Dtype used to store the recurrent WKV matrix. The bundled inference kernel requires `"float32"`.
    """
    ...
    def __post_init__(self, **kwargs):
        ...
        if self.num_attention_heads is None:
            self.num_attention_heads = inferred_num_heads
        elif self.num_attention_heads != inferred_num_heads:
            raise ValueError(
        ...
        if not isinstance(self.wkv_backend, str) or not self.wkv_backend:
            raise ValueError("`wkv_backend` must be a non-empty string.")
        if self.wkv_state_dtype != "float32":
            raise ValueError("The bundled RWKV-7 inference kernel requires `wkv_state_dtype='float32'`.")
